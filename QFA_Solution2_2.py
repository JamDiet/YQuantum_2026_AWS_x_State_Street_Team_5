...
# Set START_FROM env var to skip slow sections:
#   START_FROM=start          — run everything (default)
#   START_FROM=part2_backtest — load checkpoints, skip to walk-forward
START_FROM = os.environ.get('START_FROM', 'start')
...
if START_FROM == 'start':
    ...
    _ckpt = pickle.load(open('ckpt_quantum.pkl', 'rb'))
    Q_tr_angle, Q_te_angle = _ckpt['Q_tr_angle'], _ckpt['Q_te_angle']
    Q_tr_zz,    Q_te_zz    = _ckpt['Q_tr_zz'],    _ckpt['Q_te_zz']
    Q_tr_iqp,   Q_te_iqp   = _ckpt['Q_tr_iqp'],   _ckpt['Q_te_iqp']
    Q_tr_spec,  Q_te_spec  = _ckpt['Q_tr_spec'],  _ckpt['Q_te_spec']
    print('Loaded quantum features from ckpt_quantum.pkl')


# In[ ]:


# Save quantum features checkpoint after extraction
if START_FROM == 'start':
    pickle.dump({
        'Q_tr_angle': Q_tr_angle, 'Q_te_angle': Q_te_angle,
        'Q_tr_zz':    Q_tr_zz,    'Q_te_zz':    Q_te_zz,
        'Q_tr_iqp':   Q_tr_iqp,   'Q_te_iqp':   Q_te_iqp,
        'Q_tr_spec':  Q_tr_spec,  'Q_te_spec':  Q_te_spec,
    }, open('ckpt_quantum.pkl', 'wb'))
    print('Saved → ckpt_quantum.pkl')
...
# Save Part I results checkpoint
if START_FROM == 'start':
    pickle.dump({
        ...
        'Xtr_p2':     Xtr_p2,   'Xte_p2':   Xte_p2,
    }, open('ckpt_part1.pkl', 'wb'))
    print('Saved → ckpt_part1.pkl')
...
# Save Part II setup checkpoint (feat_A, feat_B)
if START_FROM == 'start':
    pickle.dump({'feat_A': feat_A, 'feat_B': feat_B},
               open('ckpt_part2_setup.pkl', 'wb'))
    print('Saved → ckpt_part2_setup.pkl')
...
# Load checkpoints when starting from Part II backtest
if START_FROM == 'part2_backtest':
    _a = pickle.load(open('ckpt_part1.pkl', 'rb'))
    results_p1 = _a['results_p1']
    y_test     = _a['y_test'];   y_train  = _a['y_train']
    r_test     = _a['r_test']
    mask1      = _a['mask1'];    mask2    = _a['mask2']
    all_preds  = _a['all_preds']
    X_train_s  = _a['X_train_s']; X_test_s = _a['X_test_s']
    X_train_q  = _a['X_train_q']; X_test_q = _a['X_test_q']
    Xtr_p2     = _a['Xtr_p2'];   Xte_p2   = _a['Xte_p2']
    _b = pickle.load(open('ckpt_part2_setup.pkl', 'rb'))
    feat_A = _b['feat_A']; feat_B = _b['feat_B']
    print('Loaded checkpoints — jumping to Part II walk-forward backtest')
